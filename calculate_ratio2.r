...
# Aggregate mutation counts at the gene level (across all samples)
gene_level_counts <- data %>%
  group_by(Gene_Symbol, Mutation_Type) %>%
  summarise(Total_Count = n(), .groups = "drop") %>%
  pivot_wider(names_from = Mutation_Type, values_from = Total_Count, values_fill = list(Total_Count = 0))
